/*********************************************************************************/
/* Programming Project 1:                                                        */
/*                                                                               */
/*   Header file for Programming Assignment #1.                                  */
/*                                                                               */
/*     This header file contains definitions if the light sources and material   */
/*     properties of the torus material                                          */
/*                                                                               */
/*********************************************************************************/

GLfloat shininess[]    = {100.0};
GLfloat mat_ambient [] = {0.2, 0.2, 0.2, 1.0};
GLfloat mat_specular[] = {1.0, 1.0, 1.0, 1.0};
GLfloat mat_diffuse [] = {0.8, 0.0, 0.8, 1.0};

GLfloat light1_ambient[]  = {0.0, 0.1, 0.0, 1.0};
GLfloat light1_position[] = {0.7071, 0.0, 0.7071, 0.0};
GLfloat light1_diffuse[]  = {0.0, 1.0, 0.0, 1.0};

GLfloat light0_ambient[]  = {0.1, 0.0, 0.0, 1.0};
GLfloat light0_position[] = {-0.7071, 0.0, 0.7071, 0.0};
GLfloat light0_diffuse[]  = {1.0, 0.0, 0.0, 1.0};


