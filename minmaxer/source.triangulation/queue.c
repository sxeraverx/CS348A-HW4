...
{
  queuesType *qu;
  static char *fail = { "quNew: fail on [mc]alloc.\n" };
  int i;

  if ((mode != POOL_FIXED_SIZE) && (mode != POOL_ADJUSTABLE_SIZE)) {
    printf("ERROR: pqStyleQuNew: mode inconsistent.\n");
    exit (1);
  }

  if ((qu = (queuesType *) malloc (sizeof (queuesType))) == NULL) {
    (void) fprintf (stdout, fail);
    exit (1);
  }

  qu->entry = (indexType *) calloc ((unsigned int) n+1, sizeof (indexType));
  if (qu->entry == NULL) {
    (void) fprintf (stdout, fail);
    exit (1);
  }
...
pqStyleQuNew (n, array, dummy, mode)
     ...
     int n;
     char *array;
     bool (*dummy) ();  /* in a priority queue the comparison function
...

{
  ...
  qu = (*queue.create) (5, (char *) &op, (bool *) 0);
